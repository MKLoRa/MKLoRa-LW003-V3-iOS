...
@interface MKBVUIDContentController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKBVUIDContentModel *dataModel;

@end

@implementation MKBVUIDContentController

- (void)dealloc {
    NSLog(@"MKBVUIDContentController销毁");
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"Eddystone-UID Content";
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-BV", @"MKBVUIDContentController", @"bv_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBVUIDContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVUIDContentModel alloc] init];
    }
    return _dataModel;
}
