//
//  MKBVPeriodicTimingController.m
...
@interface MKBVPeriodicTimingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKBVPeriodicTimingModel *dataModel;

@end

@implementation MKBVPeriodicTimingController

- (void)dealloc {
    NSLog(@"MKBVPeriodicTimingController销毁");
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"Periodic Scan & Timing Report";
    self.titleLabel.font = MKFont(15.f);
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-BV", @"MKBVPeriodicTimingController", @"bv_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBVPeriodicTimingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVPeriodicTimingModel alloc] init];
    }
    return _dataModel;
}
