...
@interface MKBVDeviceSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKBVDeviceSettingModel *dataModel;
...
@implementation MKBVDeviceSettingController

- (void)dealloc {
    NSLog(@"MKBVDeviceSettingController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //Local Data Sync
        MKBVSynDataController *vc = [[MKBVSynDataController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 0) {
        //Indicator Settings
        MKBVIndicatorSettingController *vc = [[MKBVIndicatorSettingController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 5 && indexPath.row == 0) {
        //Device Info
        MKBVDeviceInfoController *vc = [[MKBVDeviceInfoController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)configCurrentTimeZone:(NSInteger)index {
    ...
    [MKBVInterface bv_configTimeZone:(index - 24) sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configLowPowerPrompt:(NSInteger)prompt {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKBVInterface bv_configLowPowerPrompt:prompt sucBlock:^{
        [[MKHudManager share] hide];
        MKTextButtonCellModel *cellModel = self.section4List[0];
        cellModel.dataListIndex = prompt;
        cellModel.noteMsg = [NSString stringWithFormat:@"*When the battery is less than or equal to %@, the red LED will flash once every 30 seconds.",cellModel.dataList[prompt]];
...
- (void)configLowPowerPayload:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKBVInterface bv_configLowPowerPayloadStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)factoryReset{
    ...
    [alertView showAlertWithTitle:@"Factory Reset" message:msg notificationName:@"mk_bv_needDismissAlert"];
}
...
- (void)sendResetCommandToDevice{
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKBVInterface bv_factoryResetWithSucBlock:^{
        [[MKHudManager share] hide];
...
- (void)commandPowerOff{
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKBVInterface bv_powerOffWithSucBlock:^{
        [[MKHudManager share] hide];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
...
- (MKBVDeviceSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVDeviceSettingModel alloc] init];
    }
    return _dataModel;
}
