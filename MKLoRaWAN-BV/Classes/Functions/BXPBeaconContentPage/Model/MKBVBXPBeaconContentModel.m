...
@interface MKBVBXPBeaconContentModel ()
...
@implementation MKBVBXPBeaconContentModel
...
- (BOOL)readBXPBeaconContent {
    ...
    [MKBVInterface bv_readBXPBeaconContentWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configBXPBeaconContent {
    __block BOOL success = NO;
    [MKBVInterface bv_configBXPBeaconContent:self sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
