//
//  MKBVBXPBeaconContentController.m
...
@interface MKBVBXPBeaconContentController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKBVBXPBeaconContentModel *dataModel;

@end

@implementation MKBVBXPBeaconContentController

- (void)dealloc {
    NSLog(@"MKBVBXPBeaconContentController销毁");
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"BXP-iBeaocn Content";
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-BV", @"MKBVBXPBeaconContentController", @"bv_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBVBXPBeaconContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVBXPBeaconContentModel alloc] init];
    }
    return _dataModel;
}
