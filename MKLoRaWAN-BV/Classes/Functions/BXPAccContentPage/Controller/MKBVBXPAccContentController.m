...
@interface MKBVBXPAccContentController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKBVBXPAccContentModel *dataModel;

@end

@implementation MKBVBXPAccContentController

- (void)dealloc {
    NSLog(@"MKBVBXPAccContentController销毁");
}
...
- (void)loadSection1Datas {
    ...
    cellModel8.isOn = self.dataModel.battery;
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"BXP-ACC Content";
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-BV", @"MKBVBXPAccContentController", @"bv_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBVBXPAccContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVBXPAccContentModel alloc] init];
    }
    return _dataModel;
}
