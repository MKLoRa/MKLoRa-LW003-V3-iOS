...
@interface MKBVBleSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
mk_textSwitchCellDelegate,
MKBVBroadcastTxPowerCellDelegate>
...
@property (nonatomic, strong)MKBVBleSettingsModel *dataModel;
...
@implementation MKBVBleSettingsController

- (void)dealloc {
    NSLog(@"MKBVBleSettingsController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 5 && indexPath.row == 0 && [MKBVConnectModel shared].hasPassword) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKBVBroadcastTxPowerCell *cell = [MKBVBroadcastTxPowerCell initCellWithTableView:tableView];
...
- (void)mk_textSwitchCellStatusChanged:(BOOL)isOn index:(NSInteger)index {
    if (index == 0) {
        ...
        self.dataModel.eventNotify = isOn;
...
#pragma mark - MKBVBroadcastTxPowerCellDelegate
/*
...
- (void)bv_txPowerValueChanged:(NSInteger)txPower {
    ...
    MKBVBroadcastTxPowerCellModel *cellModel = self.section1List[0];
...
- (void)configPassword{
    ...
    [alertView showAlertWithTitle:@"Change Password" message:msg notificationName:@"mk_bv_needDismissAlert"];
}
...
- (void)setPasswordToDevice{
    ...
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKBVInterface bv_configPassword:password sucBlock:^{
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Success"];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)loadSection1Datas {
    MKBVBroadcastTxPowerCellModel *cellModel = [[MKBVBroadcastTxPowerCellModel alloc] init];
...
- (void)loadSection3Datas {
    ...
    cellModel.index = 0;
    cellModel.msg = @"Bluetooth Event Notify";
    cellModel.isOn = self.dataModel.eventNotify;
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"Bluetooth Broadcast Settings";
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-BV", @"MKBVBleSettingsController", @"bv_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBVBleSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVBleSettingsModel alloc] init];
    }
    return _dataModel;
}
