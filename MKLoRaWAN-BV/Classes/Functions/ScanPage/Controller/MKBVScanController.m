...
static NSString *const localPasswordKey = @"mk_bv_passwordKey";

static CGFloat const searchButtonHeight = 40.f;

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKBVScanController ()<UITableViewDelegate,
UITableViewDataSource,
mk_bv_centralManagerScanDelegate,
MKBVScanPageCellDelegate,
MKSearchButtonDelegate,
MKBVTabBarControllerDelegate>
...
@implementation MKBVScanController

- (void)dealloc {
    NSLog(@"MKBVScanController销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKBVCentralManager shared] stopScan];
    [MKBVCentralManager removeFromCentralList];
}
...
- (void)viewDidLoad {
    ...
    [MKBVCentralManager shared].delegate = self;
...
- (void)rightButtonMethod {
    UIViewController *vc = [[CTMediator sharedInstance] CTMediator_LORAWAN_BV_AboutPage];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKBVScanPageCell *cell = [MKBVScanPageCell initCellWithTableView:self.tableView];
...
#pragma mark - mk_bv_centralManagerScanDelegate
- (void)mk_bv_receiveDevice:(NSDictionary *)deviceModel {
    MKBVScanPageModel *model = [MKBVScanPageModel mk_modelWithJSON:deviceModel];
...
#pragma mark - MKBVScanPageCellDelegate
- (void)bv_scanCellConnectButtonPressed:(NSInteger)index {
    [self connectDeviceWithModel:self.dataList[index]];
}

#pragma mark - MKBVTabBarControllerDelegate
- (void)mk_bv_needResetScanDelegate:(BOOL)need {
    if (need) {
        [MKBVCentralManager shared].delegate = self;
    }
...
- (void)scanTimerRun{
    ...
    [[MKBVCentralManager shared] startScan];
...
- (void)updateDataWithScanModel:(MKBVScanPageModel *)scanDataModel{
 ...
 */
- (void)filterTrackerDataWithSearchName:(MKBVScanPageModel *)scanDataModel{
    if (scanDataModel.rssi < self.buttonModel.searchRssi) {
        return;
    }
...
- (void)processTrackerData:(MKBVScanPageModel *)scanDataModel{
    //查看数据源中是否已经存在相关设备
 ...
 */
- (void)dataNoExistDataSource:(MKBVScanPageModel *)scanDataModel{
 ...
 */
- (void)dataExistDataSource:(MKBVScanPageModel *)scanDataModel {
    NSInteger currentIndex = 0;
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKBVScanPageModel *dataModel = self.dataList[i];
        if ([dataModel.macAddress isEqualToString:scanDataModel.macAddress]) {
            currentIndex = i;
            break;
        }
    }
    MKBVScanPageModel *dataModel = self.dataList[currentIndex];
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKBVScanPageModel *)scanDataModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKBVCentralManager shared] stopScan];
    if (self.scanTimer) {
    ...
    [alertView showAlertWithTitle:@"Enter password" message:msg notificationName:@"mk_bv_needDismissAlert"];
}

- (void)connectDeviceWithDataModel:(MKBVScanPageModel *)scanDataModel {
    ...
    [[MKBVConnectModel shared] connectDevice:scanDataModel.peripheral password:(scanDataModel.needPassword ? password : @"") macAddress:scanDataModel.macAddress sucBlock:^{
...
- (void)configParams {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"bv_readRecordDataDayNumKey"];
    //读取设备存储的扫描信息页面，显示的设备总共存储了多少条的扫描数据
    [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"bv_recordDataTotalSumKey"];
    
    [MKBVDatabaseManager clearDataTable];
    [MKBVDatabaseManager initDataBase];
}

- (void)pushTabBarPage {
    MKBVTabBarController *vc = [[MKBVTabBarController alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-BV", @"MKBVScanController", @"bv_scanRightAboutIcon.png") forState:UIControlStateNormal];
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKLoRaWAN-BV", @"MKBVScanController", @"bv_scan_refreshIcon.png");
    }
    return _refreshIcon;
}
