...
@interface MKBVLoRaSettingController ()<UITableViewDelegate,
...
MKBVLoRaSettingAccountCellDelegate>
...
@property (nonatomic, strong)MKBVLoRaSettingModel *dataModel;

@end

@implementation MKBVLoRaSettingController

- (void)dealloc {
    NSLog(@"MKBVLoRaSettingController销毁");
}
...
- (void)rightButtonMethod {
    ...
    if (!ValidStr([MKBVUserLoginManager shared].password)) {
        //当前未登录
        @weakify(self);
        MKIoTCloudAccountLoginAlertViewModel *viewModel = [[MKIoTCloudAccountLoginAlertViewModel alloc] init];
        viewModel.account = [MKBVUserLoginManager shared].username;
        viewModel.isHome = [MKBVUserLoginManager shared].isHome;
        viewModel.password = [MKBVUserLoginManager shared].password;
    ...
    //登录过
    [self login:[MKBVUserLoginManager shared].isHome username:[MKBVUserLoginManager shared].username password:[MKBVUserLoginManager shared].password];
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (section == 7) {
        //Account
        return ((self.dataModel.configModel.supportServerPlatform && self.dataModel.platform == 1 && ValidStr([MKBVUserLoginManager shared].password)) ? self.section6List.count : 0);
    }
    if (section == 8) {
        //Gateway EUI(Gateway ID)
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 2) {
        //mokoDevEUI
        MKBVDevEUICell *cell = [MKBVDevEUICell initCellWithTableView:tableView];
    ...
    if (indexPath.section == 7) {
        //Account
        MKBVLoRaSettingAccountCell *cell = [MKBVLoRaSettingAccountCell initCellWithTableView:tableView];
...
#pragma mark - MKBVLoRaSettingAccountCellDelegate
- (void)bv_loRaSettingAccountCell_logoutBtnPressed {
    MKIoTCloudExitAccountAlert *alert = [[MKIoTCloudExitAccountAlert alloc] init];
    BOOL isHome = [MKBVUserLoginManager shared].isHome;
    NSString *username = [MKBVUserLoginManager shared].username;
    [alert showWithAccount:username completeBlock:^{
        [[MKBVUserLoginManager shared] syncLoginDataWithHome:isHome username:username password:@""];
...
- (void)login:(BOOL)isHome username:(NSString *)username password:(NSString *)password {
    ...
    [[MKIoTLoginService share] loginWithUsername:username password:password isHome:isHome sucBlock:^(id returnData) {
        ...
        [[MKBVUserLoginManager shared] syncLoginDataWithHome:isHome username:username password:password];
...
- (void)addDeviceToCloud:(NSString *)token {
    ...
    MKBVCreateLoRaDeviceModel *createModel = [[MKBVCreateLoRaDeviceModel alloc] init];
    createModel.macAddress = [MKBVConnectModel shared].macAddress;
    createModel.isHome = [MKBVUserLoginManager shared].isHome;
    createModel.gwId = self.dataModel.gatewayEUI;
    createModel.region = self.dataModel.region;
    createModel.username = [MKBVUserLoginManager shared].username;
    [[MKBVNetworkService share] addLoRaDeviceToCloud:createModel token:token sucBlock:^(id returnData) {
        [[MKHudManager share] hide];
        [self saveDataToDevice];
    } failBlock:^(NSError *error) {
        [[MKHudManager share] hide];
...
- (void)loadSection1Datas {
    MKBVDevEUICellModel *cellModel = [[MKBVDevEUICellModel alloc] init];
    NSString *macAddress = [[MKBVConnectModel shared].macAddress stringByReplacingOccurrencesOfString:@":" withString:@""];
...
- (void)loadSection6Datas {
    MKBVLoRaSettingAccountCellModel *cellModel = [[MKBVLoRaSettingAccountCellModel alloc] init];
    cellModel.account = [MKBVUserLoginManager shared].username;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-BV", @"MKBVLoRaSettingController", @"bv_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBVLoRaSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVLoRaSettingModel alloc] init];
    }
    return _dataModel;
}
