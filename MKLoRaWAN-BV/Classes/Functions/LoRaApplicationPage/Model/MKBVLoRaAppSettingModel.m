...
@interface MKBVLoRaAppSettingModel ()
...
@implementation MKBVLoRaAppSettingModel
...
- (BOOL)readTimeSyncInterval {
    ...
    [MKBVInterface bv_readLorawanTimeSyncIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTimeSyncInterval {
    __block BOOL success = NO;
    [MKBVInterface bv_configTimeSyncInterval:[self.timeSyncInterval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readNetworkCheckInterval {
    __block BOOL success = NO;
    [MKBVInterface bv_readLorawanNetworkCheckIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configNetworkCheckInterval {
    __block BOOL success = NO;
    [MKBVInterface bv_configLorawanNetworkCheckInterval:[self.checkInterval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
