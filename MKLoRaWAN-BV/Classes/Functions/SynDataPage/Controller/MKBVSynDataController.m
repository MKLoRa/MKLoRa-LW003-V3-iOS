...
@interface MKBVSynDataController ()<UITableViewDelegate,
UITableViewDataSource,
MFMailComposeViewControllerDelegate,
mk_bv_storageDataDelegate>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)NSMutableArray *dataList;

@property (nonatomic, strong)MKBVSynTableHeaderView *headerView;
...
@implementation MKBVSynDataController

- (void)dealloc {
    NSLog(@"MKBVSynDataController销毁");
    if (self.dataTimer) {
    ...
    [[MKBVCentralManager shared] notifyStorageData:NO];
}
...
- (void)viewDidLoad {
    ...
    [[MKBVCentralManager shared] notifyStorageData:YES];
...
- (void)leftButtonMethod {
    ...
    [MKBVInterface bv_pauseSendLocalData:YES sucBlock:^{
        [self startBackTimer];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [MKBVSynDataCell fetchCellHeight:self.dataList[indexPath.row]];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKBVSynDataCell *cell = [MKBVSynDataCell initCellWithTableView:tableView];
...
- (void)startButtonPressed {
    ...
    [MKBVInterface bv_readNumberOfDaysStoredData:[self.headerView.textField.text integerValue] sucBlock:^{
        self.isMaxCount = ([self.headerView.textField.text integerValue] == 65535);
        [[NSUserDefaults standardUserDefaults] setValue:self.headerView.textField.text forKey:@"bv_readRecordDataDayNumKey"];
...
- (void)synButtonPressed {
    ...
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKBVInterface bv_pauseSendLocalData:!self.headerView.synButton.selected sucBlock:^{
        [[MKHudManager share] hide];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
...
- (void)emptyButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKBVInterface bv_clearAllDatasWithSucBlock:^{
        [[MKHudManager share] hide];
...
- (void)readDataFromLocal {
    ...
    [MKBVDatabaseManager readDataListWithSucBlock:^(NSArray<NSDictionary *> * _Nonnull dataList) {
        [[MKHudManager share] hide];
...
- (void)processStatus {
    ...
    if (self.dataList.count > 0) {
        ...
        self.headerView.emptyButton.topIcon.image = LOADICON(@"MKLoRaWAN-BV", @"MKBVSynDataController", @"bv_delete_disableIcon.png");
        
        self.headerView.exportButton.enabled = NO;
        self.headerView.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-BV", @"MKBVSynDataController", @"bv_export_disableIcon.png");
    }
...
- (void)startButtonMethod {
    ...
    self.headerView.synButton.topIcon.image = LOADICON(@"MKLoRaWAN-BV", @"MKBVSynDataController", @"bv_sync_enableIcon.png");
    [self.headerView.synButton.topIcon.layer removeAnimationForKey:synIconAnimationKey];
    //开始旋转
...
- (void)syncButtonStateUpdate {
    ...
    if (self.headerView.synButton.selected) {
        ...
        self.headerView.emptyButton.topIcon.image = (enable ? LOADICON(@"MKLoRaWAN-BV", @"MKBVSynDataController", @"bv_delete_enableIcon.png") : LOADICON(@"MKLoRaWAN-BV", @"MKBVSynDataController", @"bv_delete_disableIcon.png"));
        
        self.headerView.exportButton.enabled = enable;
        self.headerView.exportButton.topIcon.image = (enable ? LOADICON(@"MKLoRaWAN-BV", @"MKBVSynDataController", @"bv_export_enableIcon.png") : LOADICON(@"MKLoRaWAN-BV", @"MKBVSynDataController", @"bv_export_disableIcon.png"));
...
- (void)emptyButtonMethod {
    ...
    self.headerView.emptyButton.topIcon.image = LOADICON(@"MKLoRaWAN-BV", @"MKBVSynDataController", @"bv_delete_enableIcon.png");
    
    self.headerView.exportButton.enabled = YES;
    self.headerView.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-BV", @"MKBVSynDataController", @"bv_export_enableIcon.png");
}
...
- (void)saveDataToLocal {
    ...
    [MKBVDatabaseManager insertDataList:self.dataList sucBlock:^{
        [[MKHudManager share] hide];
        [super leftButtonMethod];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)addDataTimer {
    ...
    dispatch_source_set_event_handler(self.dataTimer, ^{
        @strongify(self);
        moko_dispatch_main_safe(^{
            [MKBVInterface bv_readIndicatorSettingsWithSucBlock:nil failedBlock:nil];
        });
    });
...
- (MKBVSynTableHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[MKBVSynTableHeaderView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 100.f)];
