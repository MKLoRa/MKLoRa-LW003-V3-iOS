...
@interface MKBVTLMContentController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKBVTLMContentModel *dataModel;

@end

@implementation MKBVTLMContentController

- (void)dealloc {
    NSLog(@"MKBVTLMContentController销毁");
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"Eddystone-TLM Content";
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-BV", @"MKBVTLMContentController", @"bv_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBVTLMContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVTLMContentModel alloc] init];
    }
    return _dataModel;
}
