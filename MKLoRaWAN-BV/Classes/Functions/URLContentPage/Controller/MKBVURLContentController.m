...
@interface MKBVURLContentController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKBVURLContentModel *dataModel;

@end

@implementation MKBVURLContentController

- (void)dealloc {
    NSLog(@"MKBVURLContentController销毁");
}
...
- (void)loadSection1Datas {
    ...
    cellModel1.isOn = self.dataModel.measured;
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"Eddystone-URL Content";
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-BV", @"MKBVURLContentController", @"bv_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBVURLContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVURLContentModel alloc] init];
    }
    return _dataModel;
}
