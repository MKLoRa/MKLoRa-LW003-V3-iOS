...
@interface MKBVDebuggerController ()<UITableViewDelegate,
UITableViewDataSource,
MFMailComposeViewControllerDelegate,
mk_bv_centralManagerLogDelegate,
MKBVDebuggerCellDelegate>
...
@property (nonatomic, strong)MKBVDebuggerButton *deleteButton;

@property (nonatomic, strong)MKBVDebuggerButton *exportButton;
...
@implementation MKBVDebuggerController

- (void)dealloc {
    NSLog(@"MKBVDebuggerController销毁");
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_bv_stopDebuggerMode" object:nil];
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_bv_startDebuggerMode" object:nil];
...
- (void)leftButtonMethod {
    if ([MKBVCentralManager shared].connectStatus != mk_bv_centralConnectStatusConnected) {
        //当前设备已经断开连接，则返回扫描页面
    ...
    //当前设备正常连接，返回上一级页面
    self.leftAction = YES;
    [[MKBVCentralManager shared] notifyLogData:NO];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKBVDebuggerCell *cell = [MKBVDebuggerCell initCellWithTableView:tableView];
...
#pragma mark - MKBVDebuggerCellDelegate
- (void)bv_debuggerCellSelectedChanged:(NSInteger)index selected:(BOOL)selected {
    ...
    MKBVDebuggerCellModel *cellModel = self.dataList[index];
...
- (void)deviceDisconnect {
    ...
    [self.syncIcon.layer removeAnimationForKey:@"mk_bv_syncAnimation"];
    self.isLoading = NO;
...
- (void)startButtonPressed {
    ...
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-BV", @"MKBVDebuggerController", @"bv_delete_disableIcon.png");
    self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-BV", @"MKBVDebuggerController", @"bv_export_disableIcon.png");
    ...
    [self.syncIcon.layer addAnimation:[MKCustomUIAdopter refreshAnimation:1.f]
                               forKey:@"mk_bv_syncAnimation"];
}

- (void)deleteButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKBVLogDatabaseManager deleteDatasWithMacAddress:self.macAddress sucBlock:^{
        NSMutableArray *tempList = [NSMutableArray array];
        for (NSInteger i = 0; i < self.dataList.count; i ++) {
            MKBVDebuggerCellModel *cellModel = self.dataList[i];
...
- (void)readDataFromLocal {
    ...
    [MKBVLogDatabaseManager readLocalLogsWithMacAddress:self.macAddress sucBlock:^(NSArray<NSDictionary *> * _Nonnull logList) {
        [[MKHudManager share] hide];
...
- (void)loadTableViewDatas:(NSArray *)logList {
    ...
    for (NSInteger i = 0; i < logList.count; i ++) {
        ...
        MKBVDebuggerCellModel *cellModel = [[MKBVDebuggerCellModel alloc] init];
...
- (void)saveDataListToLocal {
    ...
    [MKBVLogDatabaseManager insertLogDatas:tempList macAddress:self.macAddress sucBlock:^{
        [[MKHudManager share] hide];
        if (self.leftAction) {
...
- (void)updateTopButtonState {
    BOOL enable = NO;
    for (MKBVDebuggerCellModel *cellModel in self.dataList) {
        if (cellModel.selected) {
            enable = YES;
            break;
        }
    ...
    if (enable) {
        //选中了，可以删除或者导出
        self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-BV", @"MKBVDebuggerController", @"bv_delete_enableIcon.png");
        self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-BV", @"MKBVDebuggerController", @"bv_export_enableIcon.png");
        return;
    }
    //不存在选中的
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-BV", @"MKBVDebuggerController", @"bv_delete_disableIcon.png");
...
- (void)showTipsAlert:(NSString *)msg {
    ...
    [alertView showAlertWithTitle:@"Tips!" message:msg notificationName:@"mk_bv_needDismissAlert"];
}
...
- (UIImageView *)syncIcon {
    if (!_syncIcon) {
        ...
        _syncIcon.image = LOADICON(@"MKLoRaWAN-BV", @"MKBVDebuggerController", @"bv_sync_disableIcon.png");
    }
    return _syncIcon;
}

- (MKBVDebuggerButton *)deleteButton {
    if (!_deleteButton) {
        _deleteButton = [[MKBVDebuggerButton alloc] init];
        _deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-BV", @"MKBVDebuggerController", @"bv_delete_disableIcon.png");
...
- (MKBVDebuggerButton *)exportButton {
    if (!_exportButton) {
        _exportButton = [[MKBVDebuggerButton alloc] init];
        _exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-BV", @"MKBVDebuggerController", @"bv_export_disableIcon.png");
