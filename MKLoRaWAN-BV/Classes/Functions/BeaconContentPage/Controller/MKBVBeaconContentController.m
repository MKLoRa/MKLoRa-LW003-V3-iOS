...
@interface MKBVBeaconContentController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKBVBeaconContentModel *dataModel;

@end

@implementation MKBVBeaconContentController

- (void)dealloc {
    NSLog(@"MKBVBeaconContentController销毁");
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"iBeaocn Content";
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-BV", @"MKBVBeaconContentController", @"bv_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBVBeaconContentModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVBeaconContentModel alloc] init];
    }
    return _dataModel;
}
