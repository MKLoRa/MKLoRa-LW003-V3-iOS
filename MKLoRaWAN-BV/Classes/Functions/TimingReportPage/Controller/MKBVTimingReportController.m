...
@interface MKBVTimingReportController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKBVTimingReportModel *dataModel;

@end

@implementation MKBVTimingReportController

- (void)dealloc {
    NSLog(@"MKBVTimingReportController销毁");
}
...
- (void)bv_addButtonPressed:(NSInteger)index {
    ...
    if (index == 1) {
        ...
        MKBVReportTimePointCellModel *cellModel = [[MKBVReportTimePointCellModel alloc] init];
        cellModel.type = 1;
        cellModel.index = self.section4List.count;
        cellModel.msg = [NSString stringWithFormat:@"Time Point %ld",(long)(self.section4List.count + 1)];
        cellModel.hourIndex = 0;
        cellModel.timeSpaceIndex = 0;
...
- (void)bv_cellDeleteButtonPressed:(NSInteger)type index:(NSInteger)index {
    ...
    if (type == 1) {
        //Report Time Point
        if (index > self.section4List.count - 1) {
            return;
        }
        [self.section4List removeObjectAtIndex:index];
        
        for (NSInteger i = 0; i < self.section4List.count; i ++) {
            MKBVReportTimePointCellModel *cellModel = self.section4List[i];
            cellModel.index = i;
            cellModel.msg = [NSString stringWithFormat:@"Time Point %ld",(long)(i + 1)];
        }
...
- (void)bv_hourButtonPressed:(NSInteger)type index:(NSInteger)index {
    ...
    [self showTimePointHourPickView:type index:index];
}
...
- (void)bv_timeSpaceButtonPressed:(NSInteger)type index:(NSInteger)index {
    ...
    [self showTimePointTimeSpacePickView:type index:index];
}
...
- (void)saveDataToDevice {
    NSMutableArray *scanList = [NSMutableArray array];
    for (NSInteger i = 0; i < self.section2List.count; i ++) {
        MKBVReportTimePointCellModel *cellModel = self.section2List[i];
    ...
    for (NSInteger i = 0; i < self.section4List.count; i ++) {
        ...
        pointModel.hour = cellModel.hourIndex;
        pointModel.minuteGear = cellModel.timeSpaceIndex;
        [reportList addObject:pointModel];
    }
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    @weakify(self);
    [self.dataModel configScanPointList:scanList reportPointList:reportList sucBlock:^{
        @strongify(self);
        [[MKHudManager share] hide];
...
- (void)showTimePointHourPickView:(NSInteger)type index:(NSInteger)index {
    
    MKBVReportTimePointCellModel *cellModel = (type == 0 ? self.section2List[index] : self.section4List[index]);
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:cellModel.hourIndex block:^(NSInteger currentRow) {
        ...
        [self.tableView mk_reloadRow:index inSection:(type == 0 ? 2 : 4) withRowAnimation:UITableViewRowAnimationNone];
    }];
}

- (void)showTimePointTimeSpacePickView:(NSInteger)type index:(NSInteger)index {
    MKBVReportTimePointCellModel *cellModel = (type == 0 ? self.section2List[index] : self.section4List[index]);
...
- (void)loadSection2Datas {
    for (NSInteger i = 0; i < self.dataModel.scanPointList.count; i ++) {
        MKBVScanTimePointModel *tempModel = self.dataModel.scanPointList[i];
        MKBVReportTimePointCellModel *cellModel = [[MKBVReportTimePointCellModel alloc] init];
        cellModel.type = 0;
        cellModel.index = i;
        cellModel.msg = [NSString stringWithFormat:@"Time Point %ld",(long)(i + 1)];
...
- (void)loadSection4Datas {
    for (NSInteger i = 0; i < self.dataModel.reportPointList.count; i ++) {
        MKBVScanTimePointModel *tempModel = self.dataModel.reportPointList[i];
        MKBVReportTimePointCellModel *cellModel = [[MKBVReportTimePointCellModel alloc] init];
        cellModel.type = 1;
        cellModel.index = i;
        cellModel.msg = [NSString stringWithFormat:@"Time Point %ld",(long)(i + 1)];
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"Timing Scan & Timing Report";
    self.titleLabel.font = MKFont(15.f);
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-BV", @"MKBVTimingReportController", @"bv_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBVTimingReportModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVTimingReportModel alloc] init];
    }
    return _dataModel;
}
