...
@interface MKBVFilterByURLModel ()
...
@implementation MKBVFilterByURLModel
...
- (BOOL)readFilterStatus {
    ...
    [MKBVInterface bv_readFilterByURLStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKBVInterface bv_configFilterByURLStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readURLContent {
    __block BOOL success = NO;
    [MKBVInterface bv_readFilterByURLContentWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configURLContent {
    __block BOOL success = NO;
    [MKBVInterface bv_configFilterByURLContent:self.url sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
