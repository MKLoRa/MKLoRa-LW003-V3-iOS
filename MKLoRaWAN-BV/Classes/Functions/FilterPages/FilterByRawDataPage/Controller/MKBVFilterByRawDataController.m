//
//  MKBVFilterByRawDataController.m
...
@interface MKBVFilterByRawDataController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKBVFilterByRawDataModel *dataModel;

@end

@implementation MKBVFilterByRawDataController

- (void)dealloc {
    NSLog(@"MKBVFilterByRawDataController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //iBeacon
        MKBVFilterByBeaconController *vc = [[MKBVFilterByBeaconController alloc] init];
        vc.pageType = mk_bv_filterByBeaconPageType_beacon;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Eddystone-UID
        MKBVFilterByUIDController *vc = [[MKBVFilterByUIDController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Eddystone-URL
        MKBVFilterByURLController *vc = [[MKBVFilterByURLController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Eddystone-TLM
        MKBVFilterByTLMController *vc = [[MKBVFilterByTLMController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 4) {
        //BXP-iBeacon
        MKBVFilterByBeaconController *vc = [[MKBVFilterByBeaconController alloc] init];
        vc.pageType = mk_bv_filterByBeaconPageType_bxpBeacon;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //BXP-Button
        MKBVFilterByBXPButtonController *vc = [[MKBVFilterByBXPButtonController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 1) {
        //BXP - Tag
        MKBVFilterByBXPTagController *vc = [[MKBVFilterByBXPTagController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 2) {
        //Other
        MKBVFilterByOtherController *vc = [[MKBVFilterByOtherController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)configDeviceInfo:(BOOL)isOn {
    ...
    [MKBVInterface bv_configFilterByBXPDeviceInfoStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configBXPAcc:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKBVInterface bv_configBXPAccFilterStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configBXPTH:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKBVInterface bv_configBXPTHFilterStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (MKBVFilterByRawDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVFilterByRawDataModel alloc] init];
    }
    return _dataModel;
}
