...
@interface MKBVFilterByTLMController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKBVFilterByTLMModel *dataModel;

@end

@implementation MKBVFilterByTLMController

- (void)dealloc {
    NSLog(@"MKBVFilterByTLMController销毁");
}
...
- (void)configFilterStatus:(BOOL)isOn {
    ...
    [MKBVInterface bv_configFilterByTLMStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configTLMVersion:(NSInteger)version {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKBVInterface bv_configFilterByTLMVersion:version sucBlock:^{
        [[MKHudManager share] hide];
...
- (MKBVFilterByTLMModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVFilterByTLMModel alloc] init];
    }
    return _dataModel;
}
