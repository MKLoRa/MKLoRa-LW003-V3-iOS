...
@interface MKBVFilterByTLMModel ()
...
@implementation MKBVFilterByTLMModel
...
- (BOOL)readFilterStatus {
    ...
    [MKBVInterface bv_readFilterByTLMStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKBVInterface bv_configFilterByTLMStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readTLMVersion {
    __block BOOL success = NO;
    [MKBVInterface bv_readFilterByTLMVersionWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTLMVersion {
    __block BOOL success = NO;
    [MKBVInterface bv_configFilterByTLMVersion:self.tlm sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
