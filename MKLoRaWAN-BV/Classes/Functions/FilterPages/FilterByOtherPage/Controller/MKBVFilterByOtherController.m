...
@interface MKBVFilterByOtherController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKBVFilterEditSectionHeaderViewDelegate,
MKBVFilterByRawDataCellDelegate,
...
@property (nonatomic, strong)MKBVFilterByOtherModel *dataModel;

@end

@implementation MKBVFilterByOtherController

- (void)dealloc {
    NSLog(@"MKBVFilterByOtherController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        MKBVFilterEditSectionHeaderView *headerView = [MKBVFilterEditSectionHeaderView initHeaderViewWithTableView:tableView];
        MKBVFilterEditSectionHeaderViewModel *model = [[MKBVFilterEditSectionHeaderViewModel alloc] init];
        model.index = 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKBVFilterByRawDataCell *cell = [MKBVFilterByRawDataCell initCellWithTableView:tableView];
...
#pragma mark - MKBVFilterEditSectionHeaderViewDelegate
/// 加号点击事件
/// @param index 所在index
- (void)mk_bv_filterEditSectionHeaderView_addButtonPressed:(NSInteger)index {
    ...
    MKBVFilterByRawDataCellModel *cellModel = [[MKBVFilterByRawDataCellModel alloc] init];
    if (cellModelIndex == 0) {
        //Condition A
...
#pragma mark - MKBVFilterByRawDataCellDelegate
/// 输入框内容发生改变
/// @param textType 哪个输入框发生改变了
/// @param index 当前cell所在的row
/// @param textValue 当前textField内容
- (void)bv_rawFilterDataChanged:(mk_bv_filterRawAdvDataTextType)textType
                          index:(NSInteger)index
                      textValue:(NSString *)textValue {
    if (index >= self.section1List.count) {
        return;
    }
    MKBVFilterByRawDataCellModel *cellModel = self.section1List[index];
    if (textType == mk_bv_filterRawAdvDataTextTypeDataType) {
        //过滤类型输入框内容发生改变
        cellModel.dataType = textValue;
        return;
    }
    if (textType == mk_bv_filterRawAdvDataTextTypeMinIndex) {
        //开始过滤的Byte索引输入框发生改变
        cellModel.minIndex = textValue;
        return;
    }
    if (textType == mk_bv_filterRawAdvDataTextTypeMaxIndex) {
        //截止过滤的Byte索引输入框发生改变
        cellModel.maxIndex = textValue;
        return;
    }
    if (textType == mk_bv_filterRawAdvDataTextTypeRawDataType) {
        //过滤内容输入框发生改变
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        MKBVFilterByRawDataCellModel *cellModel = self.section1List[i];
        MKBVFilterRawAdvDataModel *model = [[MKBVFilterRawAdvDataModel alloc] init];
...
- (mk_bv_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_bv_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_bv_filterByOther_AB;
        }
        return mk_bv_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_bv_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_bv_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_bv_filterByOther_AOrBOrC;
        }
    }
    return mk_bv_filterByOther_AOrBOrC;
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"Other Filter";
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-BV", @"MKBVFilterByOtherController", @"bv_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBVFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
