...
@interface MKBVFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKBVFilterByMacModel *dataModel;

@end

@implementation MKBVFilterByMacController

- (void)dealloc {
    NSLog(@"MKBVFilterByMacController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-BV", @"MKBVFilterByMacController", @"bv_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBVFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBVFilterByMacModel alloc] init];
    }
    return _dataModel;
}
