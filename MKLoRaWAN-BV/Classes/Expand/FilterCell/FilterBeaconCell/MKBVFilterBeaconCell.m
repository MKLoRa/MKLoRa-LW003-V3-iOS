...
@implementation MKBVFilterBeaconCellModel
@end

@interface MKBVFilterBeaconCell ()
...
@implementation MKBVFilterBeaconCell

+ (MKBVFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKBVFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBVFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKBVFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBVFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKBVFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKBVFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_bv_beaconMinValueChanged:index:)]) {
                [self.delegate mk_bv_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_bv_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_bv_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
