...
@implementation MKBVReportTimePointCellModel
@end

@interface MKBVReportTimePointCell ()
...
@implementation MKBVReportTimePointCell

+ (MKBVReportTimePointCell *)initCellWithTableView:(UITableView *)tableView {
    MKBVReportTimePointCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBVReportTimePointCellIdenty"];
    if (!cell) {
        cell = [[MKBVReportTimePointCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBVReportTimePointCellIdenty"];
    }
    return cell;
}
...
- (void)deleteButtonPressed {
    if ([self.delegate respondsToSelector:@selector(bv_cellDeleteButtonPressed:index:)]) {
        [self.delegate bv_cellDeleteButtonPressed:self.dataModel.type index:self.dataModel.index];
    }
}
...
- (void)hourButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(bv_hourButtonPressed:index:)]) {
        [self.delegate bv_hourButtonPressed:self.dataModel.type index:self.dataModel.index];
    }
}
...
- (void)timeSpaceButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(bv_timeSpaceButtonPressed:index:)]) {
        [self.delegate bv_timeSpaceButtonPressed:self.dataModel.type index:self.dataModel.index];
    }
}

- (void)contentPanelTapAction {
    if (self.contentPanel.frame.origin.x == 0) {
        if ([self.delegate respondsToSelector:@selector(bv_cellTapAction)]) {
            [self.delegate bv_cellTapAction];
        }
        return;
    }
    if (self.contentPanel.frame.origin.x < 0){
...
- (void)swipeEventBeTriggered:(UISwipeGestureRecognizer *)swipeGesture{
    if ([self.delegate respondsToSelector:@selector(bv_cellResetFrame)]) {
        [self.delegate bv_cellResetFrame];
    }
...
#pragma mark - setter
- (void)setDataModel:(MKBVReportTimePointCellModel *)dataModel {
    _dataModel = nil;
...
- (NSMutableArray *)timeSpaceList {
    if (!_timeSpaceList) {
        _timeSpaceList = [NSMutableArray arrayWithObjects:@"00",@"10",@"20",@"30",@"40",@"50", nil];
    }
